...
import { startPayment } from "../action";
...
const page = async ({ params }: any) => {
  ...
  const plan = myPlan
    ...
    .charAt(0)
    .toUpperCase()
    .concat(myPlan.substring(1));

  let cost: any = readData.find((el) => {
    return el.name === plan;
  });

  console.log("my Cost: ", cost.price);
  ...
  const createAccount = async (data: FormData) => {
    ...
    await startPayment(email, cost).then((res: any) => {
      console.log(res.data.data?.authorization_url);

      // location.assign(res.data.data?.authorization_url);
